...
public class CgdServiceImpl implements CgdService{
	...
	public String insertYycgd(String useryyid, String year, YycgdCustom yycgdCustom) throws Exception {
		//采购单编号
		 String bm = new SimpleDateFormat("yyyyMMddHHmmss").format(new Date());
	...
	public List<YycgdmxCustom> findAddYycgdmxList(String useryyid, String yycgdid, YycgdQueryVo yycgdQueryVo)
			throws Exception {
		//根据医院id得到医院区域id
		Useryy useryy_1=useryyMapper.selectByPrimaryKey(useryyid);
		String dq=useryy_1.getDq();//区域id
		Useryy useryy_2=yycgdQueryVo.getUseryy();
		if(useryy_2==null){
			useryy_2=new Useryy();
		}
		useryy_2.setDq(dq);
		yycgdQueryVo.setUseryy(useryy_2);
	...
	//根据采购单id和药品id查询采购单明细
		...
		public void deleteYycgdmx(String yycgdid, String ypxxid) throws Exception {
			...
			yycgdmx.setBusinessyear(businessyear);
			String id=yycgdmx.getId();
			yycgdmxMapper.deleteByPrimaryKey(id);
